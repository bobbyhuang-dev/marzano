...
- `lib/releases.ts` — the changelog (`RELEASES`, newest first) and `releasesSince`. **The top entry is the version this build calls itself**: a deploy that adds no entry announces nothing, so add one only when a change is worth telling readers about, and write it for them rather than pasting the commit subject.
